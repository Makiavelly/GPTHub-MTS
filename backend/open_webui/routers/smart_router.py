...
def _format_provider_error(data, status: int | None = None) -> str:
    if isinstance(data, dict):
        ...
        if message:
            return f'{message} (status={status})' if status is not None else str(message)

        if data:
            compact = json.dumps(data, ensure_ascii=False)
            return f'{compact} (status={status})' if status is not None else compact
...
def _extract_choice_content(data: dict, *, op: str) -> str:
    choices = data.get('choices')
    if not choices:
        raise RuntimeError(f'{op} failed: {_format_provider_error(data)}')

    message = choices[0].get('message', {})
    content = message.get('content')
    if content is None:
        raise RuntimeError(f'{op} failed: missing message content in provider response')
...
async def _decompose(user_text: str, has_image: bool, api_key: str) -> list[dict]:
    ...
    if 'choices' not in data:
        detail = _format_provider_error(data, status)
        log.error(f"[smart_router] decomposer returned no 'choices': {detail}")
        raise RuntimeError(f'decomposer failed: {detail}')

    raw = _extract_choice_content(data, op='decomposer')
...
async def _run_image_generation(prompt: str, api_key: str) -> str:
    ...
    if 'data' not in data or not data['data']:
        raise RuntimeError(f'image_generation failed: {_format_provider_error(data, status)}')
...
async def _run_chat(messages: list, model: str, api_key: str) -> str:
    ...
    if 'choices' not in data:
        detail = _format_provider_error(data, status)
        log.error(f'[smart_router] chat model={model} returned no choices: {detail}')
        raise RuntimeError(f"model '{model}' failed: {detail}")

    return _extract_choice_content(data, op=f"model '{model}'")
...
def _prepare_messages(payload: dict, user_id: str = '') -> list[dict]:
...
async def _execute_route(payload: dict, api_key: str, user_id: str = '') -> tuple[list[str], str]:
    """
    Execute the smart-router pipeline and return:
    - plan lines suitable for streaming to the client
    - final combined body for leaf tasks
    """
    api_key = api_key.removeprefix('Bearer ').strip()
    log.info(f'[smart_router] api_key preview: {api_key[:8]!r}... (len={len(api_key)})')

    messages = _prepare_messages(payload, user_id=user_id)
    ...
    return plan_lines, body


async def complete_route(payload: dict, api_key: str, user_id: str = '') -> dict:
    plan_lines, body = await _execute_route(payload, api_key, user_id=user_id)
    content = f'{"".join(plan_lines)}{body}' if plan_lines else body
    return {
        'id': 'chatcmpl-auto',
